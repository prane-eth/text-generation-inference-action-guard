                        ...
                        if tool_calls:
                            for i, raw in enumerate(tool_calls):
                                ...
                                if decision == ActionGuardDecision.BLOCK:
                                    # mark the tool call as unsafe instead of blocking
                                    tc.is_safe = False
                                    # replace the raw entry in the parsed message with the annotated version
                                    try:
                                        if isinstance(raw, dict):
                                            choice.message.tool_calls[i]["is_safe"] = False
                                        else:
                                            choice.message.tool_calls[i] = tc.dict()
                                    except Exception:
                                        # best effort: set attribute on object if possible
                                        try:
                                            setattr(raw, "is_safe", False)
                                        except Exception:
                                            pass
            return chat
        else:
                        ...
                        for choice in response.choices:
                            ...
                            if delta is not None and getattr(delta, "tool_calls", None):
                                for raw in delta.tool_calls:
                                    ...
                                    decision = action_guard(tc)
                                    if decision == ActionGuardDecision.BLOCK:
                                        # annotate unsafe tool call instead of blocking
                                        tc.is_safe = False
                                        try:
                                            if isinstance(raw, dict):
                                                raw["is_safe"] = False
                                                ...
                                                idx = delta.tool_calls.index(raw)
                                                delta.tool_calls[idx] = tc.dict()
                                        except Exception:
                                            try:
                                                setattr(raw, "is_safe", False)
                                            except Exception:
